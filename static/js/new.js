function movieListApiCall(pagenum) {
  ...
  document.getElementById("search").addEventListener("click", (e) => {
    ...
    // console.log(api_obj);
    fetch("http://www.omdbapi.com/?" + new URLSearchParams(api_obj))
        ...
        if (data.Response == "True") {
          data.Search.forEach((element) => {
            document.getElementById(
              "movie_list"
            ).innerHTML += `<div class="col-lg-3 mb-4 mt-2">
      <div class="card">
  ...
  fetch("http://www.omdbapi.com/?" + new URLSearchParams(api_obj))
    ...
    .then((data) => {
      ...
      document.getElementById("release_date").innerHTML =
        `<span class='text-muted'>Release Data : </span>` + data.Released;
      document.getElementById("runtime").innerHTML =
        `<span class='text-muted'>Runtime : </span>` + data.Runtime;
      document.getElementById("genre").innerHTML =
        `<span class='text-muted'>Genre : </span>` + data.Genre;
      document.getElementById("director").innerHTML =
        `<span class='text-muted'>Director : </span>` + data.Director;
      document.getElementById("imdb_rating").innerHTML =
        `<span class='text-muted'>IMDb Rating : </span>` + data.imdbRating;
      document.getElementById("poster").src = data.Poster;
      document.getElementById("plot").innerHTML =
        `<span class='text-muted'>Plot : </span>` + data.Plot;
    });
}
